...
- 新增 `Interactable` 组件，用于承载交互提示文本和交互响应文本。
- 玩家场景新增 `Interactor` 检测区，用于追踪附近可交互物。
- 主场景新增 `TestSign` 临时交互物，作为 Phase 2 的首个验证对象。
- 主场景新增 `CompanionNpc`，用于验证 NPC 交谈交互。
- 主场景新增 `PickupCharm`，用于验证一次性拾取物交互和交互后禁用。
- 新增 `InteractionUI`，可显示接近提示和简单文字框。
- 按 `E` 触发交互；文字框打开时锁定玩家移动，再按 `E` 或 `Esc` 关闭并恢复控制。
...
状态：第一轮探索交互验证完成。已通过 Godot 4.7.1 headless 项目加载和短运行验证；当前同一套系统已覆盖调查物、NPC 对话和一次性拾取物。后续应在编辑器内 F5 手动检查提示位置、文字框尺寸和锁定手感。
